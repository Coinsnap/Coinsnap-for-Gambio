...
use GXModules\Core\Settings\Struct\Settings;
use Coinsnap\Result\TransactionState;
use GXModules\Shop\Classes\Model\CoinsnapTransactionModel;
	
    class CoinsnapOrderActionController extends AdminHttpViewController	{
	...
	protected $settings;        //  @var Settings $settings
        protected $transactionModel;//  @var CoinsnapTransactionModel $transactionModel
	...
	*/
	public function __construct(HttpContextReaderInterface $httpContextReader, HttpResponseProcessorInterface $httpResponseProcessor, ContentViewInterface $defaultContentView) {
            ...
            $this->transactionModel = new CoinsnapTransactionModel();
	...
	/**
		...
		public function actionChangeTransactionStatus(): HttpControllerResponse
		{
			...
			try {
				if (strtolower($transaction->getState()) === strtolower($transactionStateProcessing)) {
				...
				return new HttpControllerResponse(
					sprintf('Transaction should be in state %s', $transactionStateProcessing)
				);
			} catch (\Exception $e) {
		...
		public function actionDownloadFile()
		{
			...
			$transactionStateFulfill = CoinsnapTransactionModel::TRANSACTION_STATE_FULFILL;
			
			$allowedStates = [
				$transactionStateFulfill,
				CoinsnapTransactionModel::TRANSACTION_STATE_REFUNDED,
				CoinsnapTransactionModel::TRANSACTION_STATE_PARTIALLY_REFUNDED,
				CoinsnapTransactionModel::TRANSACTION_STATE_PAID
			];
		...
		public function actionRefund()
		{
			...
			$transactionStatePaid = CoinsnapTransactionModel::TRANSACTION_STATE_PAID;
			if (
