...
class CoinsnapCheckoutPaymentModulesThemeContentView extends CoinsnapCheckoutPaymentModulesThemeContentView_parent
{
	...
	$this->set_content_template('GXModules/Coinsnap/CoinsnapPayment/Shop/Themes/All/checkout_payment_modules.html');
	...
	if (empty($t_uninitialized_array)) {
		...
		$radio_buttons = 0;
			...
			$radio_buttons++;
                ...
                $paymentMethodConfKey = 'configuration/MODULE_PAYMENT_COINSNAP_';
		$query = xtc_db_query("SELECT * FROM `gx_configurations` WHERE `key` LIKE '" . xtc_db_input($paymentMethodConfKey) . "%' AND  `type` = 'switcher' AND `value` = 'true'");
		...
		while ($row = mysqli_fetch_assoc($query)) {
                ...
                $array = [];
		foreach ($selection as $t_key => $t_method_array) {
                    if (strpos(strtolower($t_method_array['id']), 'coinsnap') !== false) {
                        
                        if (strtolower($t_method_array['id']) !== 'coinsnap') {
